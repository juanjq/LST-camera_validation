    ...
    with PdfPages('temp/multi_graphs.pdf') as pdf:         # name of temporal pdf
        ...
        for cluster in range(len(data[run])):
            ...
            if data_type == 'l0' and plot_run == True :
                ...
                print('Pixel ' + str(cluster) + '/' + str(len(data[run])))
            ...
            if plot_run == True:                                               # only if we want the plots
                ...
                splots[splot_i].plot(t_stepf, y_stepf, '-', lw=0.7, alpha=0.75, zorder=1, label='Fitting') # step func
...
def histogram_treshold_plot(tresholdRaw,mean,sigma,Nzeros,run,gain,voltage,neigh,dac,clusOrPx_name,run_failed,plot_run=True):
    ...
    if (plot_run == True) or (run_failed == True): # if we want the plots
        ...
        yhist, xhist, patches = plt.hist(tresholdRaw, bins = 33, alpha = 0.7, color = 'royalblue', density = True, histtype='stepfilled')
        ...
        # parameters and save
        title = 'Histogram: Gain = ' + str(gain[run]) + ',  DAC ' + str(dac[run]) + ',  Looking neighbor ' + str(neigh[run])
        title = title + ',  ' + voltage[run].replace(' ', '').replace('_', ' ') + '\n \n ' + 'Count of non fitted '
        title = title + clusOrPx_name + 's = ' + str(Nzeros) + ',  Standard deviation ' + '$\\sigma$ = ' + str(round(sigma, 2))
        plt.title(title)  
...
def cluster_treshold_plot(data, run, tresholdZero, mean, clusOrPx_name, gain, dac, voltage, neigh, run_failed, plot_run=True):
    ...
    if (plot_run == True) and (run_failed == False):                             # if we want the plots
        ...
        title = clusOrPx_name + ' 50% treshold: Gain = ' + str(gain[run]) + ',  DAC ' + str(dac[run])
        title = title + ',  Looking neighbor ' + str(neigh[run]) + ',  ' + voltage[run].replace(' ', '').replace('_', ' ')
        plt.title(title)
...
def cam_plot(x, mu, title, data_type, leftPlot=True):
    ...
    cgeom = CameraGeometry.from_name("LSTCam")          # camera geometry used from ctapipe
    cgeom = cgeom.transform_to(EngineeringCameraFrame())
    ...
    if leftPlot == True:
        ...
        camdisplay = CameraDisplay(cgeom, norm=colors.CenteredNorm(mu), title=title, image=xPixel, cmap=cmap, show_frame=False) 
    ...
    # for the right plot
        ...
        # camera plot with some parameters
        camdisplay = CameraDisplay(cgeom, norm=colors.CenteredNorm(0.4), title=title, image=xPixel, cmap=cmap, show_frame=False) 
    
    
# camera representations
def camera_repr(data, tresholdMean, tresholdFitting, mean, Nzeros, run, gain, dac, neigh, voltage,clusOrPx_name, data_type, 
                run_failed, plot_run = True):
    ...
    if (plot_run == True) and (run_failed == False):                          # if we want the plots
        ...
        title = clusOrPx_name + 's 50% treshold: Gain = ' + str(gain[run]) + ',  DAC ' + str(dac[run]) + ',  Looking neighbor '
        title = title + str(neigh[run]) + ',  ' + voltage[run].replace(' ', '').replace('_', ' ') + '\n \n (Not fitted '
        title = title + clusOrPx_name + 's are represented also in gray ' + '$\\rightarrow$ more info in right graph)'
        ...
        if data_type == 'l1':
            ...
            title = 'Mean 50% treshold = ' + str(round(mean,1)) + ' \n \n Number of ' + clusOrPx_name + 's that we could '
            title = title + 'not fit = ' + str(Nzeros) + ', the ' + str(round((Nzeros / len(data[run]) * 100), 1)) + '%'
...
def all_together_plot(data, t, run, mean, plt_limit, gain, dac, neigh, voltage, clusOrPx_name, run_failed, plot_run=True):
    ...
    if (plot_run == True) and (run_failed == False):                            # if we want the plots
        ...
        title = 'All data together: Gain = ' + str(gain[run]) + ',  DAC ' + str(dac[run]) + ',  Looking neighbor '
        title = title + str(neigh[run]) + ',  ' + voltage[run].replace(' ', '').replace('_', ' ')
        plt.title(title)
        ...
        plt.plot([], [], '.', color='royalblue', alpha=0.5, label='Each '+clusOrPx_name+' points')
        plt.legend(loc = 1)
...
def cam_plotTOT(x, title, data_type):
    ...
    # camera plot with some parameters
    camdisplay = CameraDisplay(cgeom, norm=colors.CenteredNorm(0), title=title, image=xPixel, cmap=cmap, show_frame=False) 
...
def camera_reprTOT(mean_dev, data_type, clusOrPx_name):
    ...
    for i in [0, 1, 2, 3, 4, 5]:
        ...
        title = numText[i] + ' dev: '+ clusOrPx_name + ' ' + str(index[i] + 1)
        title = title + ', with dev from mean = ' + str(round(mean_dev[index[i]], 2))
