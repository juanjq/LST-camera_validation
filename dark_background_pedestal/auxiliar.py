...
def freqFilterPedestal(f, fftPedestal):   
    ...
    ax2.axvline(MIN_FREQ, color='b', linestyle='-', linewidth=16, alpha=0.2, label='Minimum h.a. \n frequency')
...
def plot_fftsPedestal(random_pixels, RUN, subrun, time, pedestal, stdv, fftPedestal, fftStdv, meanPedestal, meanStdv,
    ...
    for index in indexes:
        ...
        label, c1 = 'Pedestal mean = ' + str(round(meanPedestal[index], 1)), 'darkorange'
        ax1.plot([0, time[time.size-1] - time[0]], [meanPedestal[index], meanPedestal[index]], '--', color=c1, label=label)
        ...
        savename = dir_graphs + 'fft_pedestal_Run' + str(RUN) + '_Subrun' + str(subrun) + '_px' + str(index)
        ...
        ax1.plot([0, time[time.size-1] - time[0]], [meanStdv[index], meanStdv[index]], '--', color='k', label=label)
        ...
        savename = dir_graphs + 'fft_stdv_Run' + str(RUN) + '_Subrun' + str(subrun) + '_px' + str(index)
        plt.savefig(savename + '.' + graphs_format, bbox_inches='tight', format=graphs_format)
